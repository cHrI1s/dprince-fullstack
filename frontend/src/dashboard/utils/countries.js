...
export function getCountryNameByCode(code){
    let found = getCountries().find(country=> country.value===code);
    if(found!==null && typeof found!=="undefined") return found.label;
    return "--";
}
...
function getCountryObjectByCode(countryCode){
    ...
    for(; i<countriesCount; i++){
        let singleCountry = countries[i];
        if(singleCountry.code===countryCode){
...
export { getCountries, getCountryObjectByCode }